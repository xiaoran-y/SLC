# Code adapted from https://github.com/pykt-team/pykt-toolkit (AKT implementation)
import math
...
class AKT(nn.Module):
    ...
    def forward(self, q_data, qa_data, target, pid_data=None):
        ...
        # Avoid boolean indexing (dynamic output shapes) for torch.compile stability.
        labels_01 = torch.where(mask, labels, torch.zeros_like(labels)).float()
        loss_vec = loss(preds, labels_01)
        loss_sum = (loss_vec * mask.float()).sum()
        return loss_sum + c_reg_loss, torch.sigmoid(preds), mask.sum()
...
def attention(q, k, v, d_k, mask, dropout, zero_pad, gamma=None):
    ...
    with torch.no_grad():
        ...
        scores_ = scores_ * mask.to(dtype=scores_.dtype, device=dev)
        distcum_scores = torch.cumsum(scores_, dim=-1)  # bs, 8, sl, sl
        disttotal_scores = torch.sum(scores_, dim=-1, keepdim=True)  # bs, 8, sl, 1
        position_effect = torch.abs(x1 - x2)[None, None, :, :].to(dtype=torch.float32)  # 1, 1, seqlen, seqlen
