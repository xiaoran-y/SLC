#!/usr/bin/env python
# coding=utf-8
# Code adapted from https://github.com/pykt-team/pykt-toolkit (LPKT implementation)
...
class LPKT(nn.Module):
    ...
    def forward(self, e_data, a_data, it_data=None, at_data=None, qtest=False):
        ...
        # Deterministic initial knowledge state (per-batch). Random init here would
        # make evaluation nondeterministic even with dropout=0.
        h_pre = torch.zeros(batch_size, self.n_question + 1, self.d_k, device=device)
        h_tilde_pre = None
        if emb_type == "qid":
